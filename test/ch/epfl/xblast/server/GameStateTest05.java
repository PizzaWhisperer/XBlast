...
import static ch.epfl.xblast.server.GameStateTestUtils.addToSet;
import static ch.epfl.xblast.server.GameStateTestUtils.asSet;
import static ch.epfl.xblast.server.GameStateTestUtils.bombDecreased;
import static ch.epfl.xblast.server.GameStateTestUtils.removeFromSet;
import static ch.epfl.xblast.server.GameStateTestUtils.xAssertEquals;
import static ch.epfl.xblast.server.GameStateTestUtils.xAssertNotEquals;
...
public class GameStateTest05 {
    ...
    public void droppedBombConflictNotOnlySamePlayer() {
        ...
        //compte les bombes qui ont été dropped par personne. 
        System.out.println("Dropped");
        assertEquals(1, asSet(PlayerID.values()).stream().map(p -> bombsDropped.stream().filter(p2 -> p2 == p).count())
                .collect(toSet()).size()); //je suis clair lol haha je vous fais chier
    }
