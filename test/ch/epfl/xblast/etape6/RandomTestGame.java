...
/**
 * Checks that the player move as in the example for the random game provided as
 * example
 */
public class RandomTestGame {
	...
	public void testPositionsRandomGame() throws InterruptedException, IOException, URISyntaxException {
		...
		BoardPainter bp = new BoardPainter(Level.DEFAULT_PALET, BlockImage.IRON_FLOOR_S);
		...
		while (!s.isGameOver()) {
			...
			GameStateDeserializer.deserializeGameState((GameStateSerializer.serialize(bp, s)));
